...
RSpec.describe Decks::ForkService do
  let(:owner)    { create(:user) }
  let(:forker)   { create(:user) }
  let!(:source)  { create(:deck, :public, user: owner) }

  subject(:result) { described_class.call(source_deck: source, user: forker) }

  it "creates a new deck belonging to the forking user" do
    expect { result }.to change(Deck, :count).by(1)
    expect(result.user).to eq(forker)
  end

  it "sets the forked deck to private" do
  ...
  it "accepts a custom name override" do
    custom = described_class.call(source_deck: source, user: forker, name: "My Copy")
  ...
  it "does not clone the password_digest" do
    ...
    forked = described_class.call(source_deck: protected_source, user: forker)
    expect(forked.password_digest).to be_nil
  end
end
